...
#input
echo " "
echo "#--Entering NetworX Tool Menu for site_2 site--#"
echo " "
echo "########################################"
...
case $choose in
	...
	fw)
	   ...
	   DATE=`date +%F`
	   MONTH=`date +%b-%y`
	   DIR=~/script/logs/site_2-Network-Tool/$MONTH/Compare-Config-Firewall_$DATE
	   FW=2.225.66.1
           mkdir -p $DIR
	   echo " "
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-Network-Tool/$MONTH/Compare-Config-Switch_$DATE
	...
	*)
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-Network-Tool/$MONTH/Compare-config-custom_$DATE
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-Network-Tool/$MONTH/ListFile-Backup_$DATE
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-Netowrk-Tool/$MONTH/Discovery_$DATE
	...
	# Input
	echo "************************************************"
	echo -n "Input the network [E.g. 2.2.2.0]:"
        read network
        echo ""
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-Network-Tool/$MONTH/SNMP_$DATE

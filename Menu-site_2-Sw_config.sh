...
case $choose in
	...
	DATE=`date +%F_%H%M`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-NAC-Sw/$MONTH/site_2-Sw-configFile_$DATE
	...
	echo " ****************************** "
         ...
         ./site_2-Sw-configFile-Alcatel.exp $address $username $password > $DIR/$address 2>&1
	...
	DATE=`date +%F_%H%M`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-NAC-Sw/$MONTH/site_2-Sw-configFileAll_$DATE
	...
	do
	   ...
	   echo "$num.Switch IP: $address ... saved"
           ./site_2-Sw_configFile-Alcatel.exp $address $password > $DIR/$address 2>&1
	...
	DATE=`date +%F_%H%M`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_2-NAC-Sw/$MONTH/site_2-Sw-configFile-verify_$DATE
	...
	echo " ****************************** "
           ...
           ./site_2-Sw-configFile-verify.exp $address $username $password > $DIR/$address 2>&1

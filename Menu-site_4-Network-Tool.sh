...
# Script name    : Menu-site_4-Network-Tool.sh
# Site           : site_4 
# Developer      : sm@Devel
#--------------------------------------------#
...
#input
echo " "
echo "#--Entering NetworX Tool Menu for site_4 site--#"
echo " "
echo "########################################"
...
case $choose in
	...
	fw)
	   ...
	   DATE=`date +%F`
	   MONTH=`date +%b-%y`
	   DIR=~/script/logs/site_4-Network-Tool/$MONTH/Compare-Config-Firewall_$DATE
	   FW=10.225.66.1
           mkdir -p $DIR
	   echo " "

	   # Prepare the job
           dir0=~/script/logs/site_4-Firewall/$MONTH 
           ls -tr ~/script/logs/site_4-Firewall/$MONTH | tail -1 > dir1
	   ls -tr ~/script/logs/site_4-Firewall/$MONTH | tail -2 | head -1 > dir2
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_4-Network-Tool/$MONTH/Compare-Config-Switch_$DATE
	...
	*)
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_4-Network-Tool/$MONTH/Compare-config-custom_$DATE
	...
	# Put the codes here...
	list_dir=`ls -tr * / ~/script/logs/Switch/$MONTH`
	num_dir=`ls -tr * / ~/script/logs/Switch/$MONTH | wc -l`
	echo "$num_dir"
	for ((i=0; i=<$num_dir; i++))
	do
	   ls_date=`ls -tr * / ~/script/logs/Switch/$MONTH | head -i | tail -1 | cut -d _ f2`
	   if ($mydate != $ls_date)
	   then 
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_4-Network-Tool/$MONTH/ListFile-Backup_$DATE
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_4-Netowrk-Tool/$MONTH/Discovery_$DATE
	...
	DATE=`date +%F`
	MONTH=`date +%b-%y`
	DIR=~/script/logs/site_4-Network-Tool/$MONTH/SNMP_$DATE
